...
MCP stdio server wrapping `@ai-native-solutions/fallhome-sdk`.

## Install
```bash
claude mcp add fallhome -- npx -y @ai-native-solutions/fallhome-mcp
```
...
- **Discoverable** — indexed on the FallMarket catalog at [sjgant80-hub.github.io/fallmarket/listing.html?id=fallhome-mcp](https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallhome-mcp)
...
- **Companion SDK** (`npm install @ai-native-solutions/fallhome-mcp-sdk`) — programmatic JavaScript API for direct integration.
- **Companion MCP server** (`claude mcp add fallhome-mcp -- npx -y @ai-native-solutions/fallhome-mcp-mcp`) — Model Context Protocol server that any Claude/Cursor/Codex client can invoke as a tool.
- **Companion HTTP API** (`docker run --rm ghcr.io/sjgant80-hub/fallhome-mcp-api`) — containerized REST endpoint for language-agnostic access.
...
- **Registry entry**: https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallhome-mcp
- **Live URL**: https://sjgant80-hub.github.io/fallhome-mcp/
- **Source**: https://github.com/sjgant80-hub/fallhome-mcp
